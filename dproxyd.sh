...
function get_subnet_mask() {
  subnet_cache="$proxy_dir/current_subnet64.txt"

  # если директории нет — создаем
  mkdir -p "$proxy_dir"

  # если файл есть и ему < 24 часов — читаем
  if [[ -f "$subnet_cache" && $(find "$subnet_cache" -mmin -lt 1440) ]]; then
    subnet_mask=$(cat "$subnet_cache")
    echo "$subnet_mask"
    return
  fi

  # иначе — генерируем новую /64 из /48
  ipv6_prefix=$(ip -6 addr | awk '{print $2}' | grep -m1 -oP '^(?!fe80)([0-9a-fA-F]{1,4}:){3}' | cut -d '/' -f1)

  hex() { printf "%x" $((RANDOM % 65536)); }

  subnet_mask="${ipv6_prefix}$(hex):$(hex):$(hex)"
  echo "$subnet_mask" > "$subnet_cache"
